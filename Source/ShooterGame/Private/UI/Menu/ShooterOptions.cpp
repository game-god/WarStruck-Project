...
void FShooterOptions::Construct(APlayerController* _PCOwner)
{
...
int32 FShooterOptions::GetOwnerUserIndex()
{
	UPlayer * Player = PCOwner->Player;
	ULocalPlayer * LocalPlayer = Player ? CastChecked<ULocalPlayer>(Player) : nullptr;
	return LocalPlayer ? LocalPlayer->ControllerId : 0;
}
...
void FShooterOptions::UpdateOptions()
{
	...
	if (PCOwner->GetNetMode() == NM_Client)
	{
		CheatsItem->bVisible = false;
	}
	else
...
{
	for (FConstPlayerControllerIterator It = PCOwner->GetWorld()->GetPlayerControllerIterator(); It; ++It)
	{
...
{
	AShooterGameState* const GameState = (PCOwner) ? Cast<AShooterGameState>(PCOwner->GetWorld()->GameState) : NULL;
	if (GameState)
	{
