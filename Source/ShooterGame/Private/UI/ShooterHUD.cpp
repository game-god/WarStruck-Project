...
AShooterHUD::AShooterHUD(const class FPostConstructInitializeProperties& PCIP) : Super(PCIP)
{
		...
		if (MyPC && MyGameState && MatchState == EShooterMatchState::Playing)
		{
			...
			if (MyPlayerState)
			{
				if (MyGameState->NumTeams > 1) // team based game
				{
					...
					Text = FString::Printf(TEXT("%d/%d"), MyPos, MyGameState->NumTeams);
				}
				else // free for all
				{
					RankedPlayerMap PlayerStateMap;
					MyGameState->GetRankedMap(0,PlayerStateMap);
					int32 MyPos = *PlayerStateMap.FindKey(MyPlayerState) + 1;
...
void AShooterHUD::DrawCrosshair()
{
	...
	if (PCOwner)
	{
		AShooterCharacter* Pawn =  Cast<AShooterCharacter>(PCOwner->GetControlledPawn());
...
{
	const float CurrentTime = GetWorld()->GetTimeSeconds();
	AShooterCharacter* MyPawn = (PlayerOwner) ? Cast<AShooterCharacter>(PlayerOwner->GetControlledPawn()) : NULL;
	if (MyPawn)
	{
...
void AShooterHUD::ShowScoreboard(bool bEnable)
{
	AShooterPlayerController* ShooterPC = Cast<AShooterPlayerController>(PlayerOwner);

	if (ShooterPC->IsGameMenuVisible() && bEnable)
	{
		return;
	}
...
void AShooterHUD::AddChatLine(const FString& ChatString)
{
	TryCreateChatWidget();
	if( ChatWidget.IsValid() == true )
	{
		ChatWidget->AddChatLine(ChatString);
	}
}
